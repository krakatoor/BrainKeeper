...
struct StroopTesting: View {
    ...
    var body: some View {
        VStack {
            TabView(selection: $viewModel.colorsViewTag) {
                
                ForEach(0..<5) { index in
                    VStack (spacing: 10){
                    ForEach(0...9, id: \.self) { index in
                        Text(viewModel.startStroopTestTimer ? colorsName.randomElement()! : colorsName[4])
                            .mainFont(size: 25)
                            .foregroundColor(viewModel.startStroopTestTimer ? colors.randomElement()! : colors[index])
                ...
                .redacted(reason: viewModel.startStroopTestTimer ? [] : .placeholder)
...
struct StroopFinish: View {
    ...
    var body: some View {
        VStack {
           ...
           Text("\(viewModel.stroopTestResult)\n\n Тест Струпа оценивает совместную работу передних частей лобных долей левого и правого полушарий. Скорость его выполнения завист от индивидуальных особенностей, поэтому никакие верменые рамки не устанавливаются. Возьмите за основу свой результат, полученый на предыдущей неделе.")
            .padding()
